...
async def process_file(file: UploadFile = File(...), department: str = Form(...)):
    ...
    checkpoints_OLP = [
        "Как вы оцениваете работу группы лаборантов-координаторов?",
        "Как вы оцениваете работу группы общелабораторного сервиса?",
        "Как вы оцениваете работу группы содержания лабораторных помещений (группы клининга)?"
    ]
    skip = False

    processed_rows = []
    for col in question_columns:
        if col in checkpoints_OLP:
            skip = False
        if skip:
            continue
        ...
        if col in checkpoints_OLP and temp_df["оценка"].iloc[0] == 0:
            skip = True
            continue
